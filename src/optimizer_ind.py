...
def write_file(filename, vals):
    ...
    # Assuming your repo's base directory is the current working directory
    repo_dir = Path.cwd().parent
    file_path = repo_dir / filename
    with open(file_path, "a") as f:
        print(
            "{:6d}\t{:.16f}\t{:.16e}\t{:.16f}\t".format(step, lr, loss, predicts),
...
def optimize_Rf_meta_fn(
    ...
    def outer_step(eta, params, meta_state, state, R):
        (loss, aux), grad = value_and_grad(outer_loss, has_aux=True)(
            eta, params, state, R
        )
    ...
    while k < opt_steps and loss > ltol:
        eta, update_params, meta_state, state, loss, pred, grad, Rf = jit(outer_step)(
            eta, params, meta_state, state, R0
        )
        ...
        if loss == 0.0:
            ...
            update_params = {key: params[key] + 1.0e-6 * grad[key] for key in keys}
            loss = 100.0
            ncount += 1
        else:
            R0 = Rf
